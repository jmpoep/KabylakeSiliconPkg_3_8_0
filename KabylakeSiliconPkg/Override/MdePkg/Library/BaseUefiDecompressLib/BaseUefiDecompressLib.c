  ...
  )
{
  ...
  UINT16  WordOfStart;
  UINT16  WordOfCount;
  UINT16  MaxTableLength;  // SICO-KBL-0009: RoyalParkOverrideContent
  ...
  MaxTableLength = (UINT16) (1U << TableBits);  // SICO-KBL-0009: RoyalParkOverrideContent
  ...
  for (;;) {
    ...
    if (CharC < 256) {
      ...
      while ((INT16) (BytesRemain) >= 0) {
        // Sd->mDstBase[Sd->mOutBuf++] = Sd->mDstBase[DataIdx++];  // SICO-KBL-0009: RoyalParkOverrideContent
        if (Sd->mOutBuf >= Sd->mOrigSize) {
          goto Done;
        }
        //
  ...
  )
{
  ...
  if (SourceSize < (CompressedSize + 8) || (CompressedSize + 8) < 8) {  // SICO-KBL-0009: RoyalParkOverrideContent
    return RETURN_INVALID_PARAMETER;
  }
